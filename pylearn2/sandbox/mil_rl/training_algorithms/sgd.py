...
OnlineSGD: Modified by Aaron Mishtal to work with models
that operate online. It now accumulates gradients on each
call to train and only performs an update after `batch_size`
calls. This class should be used with a dataset of one
example at a time.
...
class OnlineSGD(TrainingAlgorithm):
    """
    ...
    def setup(self, model, dataset):
        ...
        #self.acc_grads = T.zeros_like(grads)
        self.acc_grads = OrderedDict()
        for param in params:
            self.acc_grads[param] = sharedX(np.zeros_like(param.get_value()))
        #acc_updates = updates
        acc_updates = OrderedDict()
        acc_updates.update(OrderedDict(safe_zip([self.acc_grads[param] for param in params],
                                    [self.acc_grads[param] + grads[param] / float(self.batch_size) for param in params])))
                                   #[self.acc_grads[param] + grads[param] / float(self.batch_size) for param in params]))
        self.accumulate_gradients = function(theano_args, updates=acc_updates, mode=self.theano_function_mode)

        if self.learning_rule:
            updates.update(self.learning_rule.get_updates(
                learning_rate, self.acc_grads, lr_scalers))
        else:
            # Use standard SGD updates with fixed learning rate.
            updates.update( dict(safe_zip(params, [param - learning_rate * \
                lr_scalers.get(param, 1.) * self.acc_grads[param]
                                    for param in params])))
        updates.update( OrderedDict(safe_zip([self.acc_grads[param] for param in params],
                                      [T.zeros_like(param) for param in params])))
                                      #[sharedX(np.zeros_like(param.get_value())) for param in params])))
    ...
    def train(self, dataset):
        ...
        for batch in iterator:
            ...
            #for key in self.acc_grads.keys():
            #    print key
            #    print self.acc_grads[key].get_value()
            #print self.acc_grads[self.params[-1]].get_value()
            self.accumulate_gradients(*batch)
            #print self.acc_grads[self.params[-1]].get_value()
            #print ''
            #for acc_grad in self.acc_grads:
            #    print acc_grad.get_value()

            #print self.batch_size, self.example_count
            if self.example_count >= self.batch_size:
